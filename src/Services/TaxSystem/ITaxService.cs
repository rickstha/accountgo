...
namespace Services.TaxSystem
{
    ...
    {
        IEnumerable<Core.Domain.Tax> GetTaxes(bool includeInActive = false);
        IEnumerable<Core.Domain.TaxGroup> GetTaxGroups();
        IEnumerable<Core.Domain.ItemTaxGroup> GetItemTaxGroups();
        IEnumerable<Core.Domain.Tax> GetIntersectionTaxes(int itemId, int partyId, Core.Domain.PartyTypes partyType);
        IEnumerable<Core.Domain.Tax> GetIntersectionTaxes(int itemId, int partyId);
        decimal GetSalesLineTaxAmount(decimal quantity, decimal amount, decimal discount, IEnumerable<Core.Domain.Tax> taxes);
